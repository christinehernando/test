(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function createKeyboardEvent(type, keyOrKeyCode, target) {
        ...
        if (event.initKeyEvent) {
            ...
            var modifiersStr = (modifiers.control ? 'Control ' : '' + modifiers.alt ? 'Alt ' : '' + modifiers.shift ? 'Shift ' : '' + modifiers.meta ? 'Meta' : '').trim();
    ...
    var IS_FOCUS_PATCHED_PROP = Symbol('isFocusPatched');
    /** Ensures that a single set of matching focus and blur events occur when HTMLElement.focus() is called. */
    var FocusEventWatcher = /** @class */ (function () {
        function FocusEventWatcher(element) {
            var _a;
        ...
        FocusEventWatcher.prototype.handleEvent = function (_b) {
            var type = _b.type;
            if (type === 'focus') {
                this._focused = true;
            }
         ...
         */
        FocusEventWatcher.prototype.ensureFocusEvents = function () {
            var _a;
    ...
    function patchElementFocus(element) {
        ...
        if (!isRunningInJsDom() && (element[IS_FOCUS_PATCHED_PROP] === undefined)) {
            var originalFocus_1 = element.focus.bind(element);
            element.focus = function (options) {
                var focusEventWatcher = new FocusEventWatcher(element);
                // Sets document.activeElement. May or may not send focus + blur events
                originalFocus_1(options);
    ...
    var DomSpectator = /** @class */ (function (_super) {
        ...
        DomSpectator.prototype.triggerEventHandler = function (directiveOrSelector, eventName, eventObj, options) {
            var triggerDebugElement = this.getTriggerDebugElement(directiveOrSelector, options);
        ...
        DomSpectator.prototype.getTriggerDebugElement = function (directiveOrSelector, options) {
            var debugElement = (options === null || options === void 0 ? void 0 : options.root) ? this.getRootDebugElement() : this.debugElement;
            if (isString(directiveOrSelector)) {
                return debugElement.query(platformBrowser.By.css(directiveOrSelector));
            }
            else if (directiveOrSelector instanceof core.DebugElement) {
                return directiveOrSelector;
            }
            else {
                return debugElement.query(platformBrowser.By.directive(directiveOrSelector));
            }
        };
        DomSpectator.prototype.getRootDebugElement = function () {
            var element = this.debugElement;
            /**
    ...
    var checkProperty = function (el, prop, predicate) {
        ...
        var failing = '';
        try {
            for (var _c = __values(Object.keys(prop)), _d = _c.next(); !_d.done; _d = _c.next()) {
                var key = _d.value;
    ...
    var ɵ9 = function (el, expected, options) {
        if (options === void 0) { options = { strict: true }; }
        if (expected && Array.isArray(expected)) {
            var actual_2 = $__default['default'](el).attr('class');
            var expectedClasses_1 = expected.join(' ');
            var pass_3 = options.strict ? $__default['default'](el).hasClass(expectedClasses_1) : expected.every(function (e) { return $__default['default'](el).hasClass(e); });
    ...
    var ɵ10 = function (el, attr, val) {
        ...
        if (isObject(attr)) {
            ...
            var failing_2;
            try {
                for (var _c = __values(Object.keys(attr)), _d = _c.next(); !_d.done; _d = _c.next()) {
                    var key = _d.value;
    ...
    var ɵ17 = function (el, _b) {
        var data = _b.data, val = _b.val;
    ...
    var ɵ21 = function (actual, expected) {
        var mapToPropsAndValues = function (values, properties) {
        ...
        var actualPropsAndValues = mapToPropsAndValues(actual, actualProps);
        var expectedProps = Object.getOwnPropertyNames(expected);
        var expectedPropsAndValues = mapToPropsAndValues(expected, expectedProps);
        var pass = expectedProps.every(function (expectedProp) { return actual[expectedProp] === expected[expectedProp]; });
        var message = function () { return ("Expected element" + (pass ? ' not' : '') + " to contain properties: " + JSON.stringify(expectedPropsAndValues) + ".").concat(" Actual properties: " + JSON.stringify(actualPropsAndValues)); };
        return { pass: pass, message: message };
    };
    /**
     ...
     */
    var toBePartial = comparator(ɵ21);
    /**
     ...
     *
     */
    var toBeSelected = comparator(ɵ23);
    var ɵ24 = function (el) {
        var pass = !isHidden(el);
     ...
     *
     */
    var toBeVisible = comparator(ɵ24);
    var ɵ25 = function (el) {
     ...
     */
    var toBeFocused = comparator(ɵ25);
    var ɵ26 = function (el, expected) {
     ...
     */
    var toBeMatchedBy = comparator(ɵ26);
    var ɵ27 = function (el, selector) {
     ...
     */
    var toHaveDescendant = comparator(ɵ27);
    var ɵ28 = function (el, _b) {
        var selector = _b.selector, text = _b.text;
        var actual = $__default['default'].trim($__default['default'](el).find(selector).text());
     ...
     */
    var toHaveDescendantWithText = comparator(ɵ28);
    var ɵ29 = function (el, expected) {
        var _a;
        ...
        var actual = $__default['default'](el).val();
        var pass = (_a = coerceArray(expected)) === null || _a === void 0 ? void 0 : _a.every(function (v) { return actual.includes(v); });
    ...
    var toHaveSelectedOptions = comparator(ɵ29);
    ...
    function initialModule(options) {
        ...
        return {
            declarations: __spreadArray(__spreadArray(__spreadArray([], __read(globals.declarations)), __read(options.declarations)), __read(options.entryComponents)),
            imports: __spreadArray(__spreadArray(__spreadArray([], __read((options.disableAnimations ? [animations.NoopAnimationsModule] : []))), __read(globals.imports)), __read(options.imports)),
            providers: __spreadArray(__spreadArray(__spreadArray([], __read(globals.providers)), __read(options.providers)), __read(options.mocks.map(function (type) { return options.mockProvider(type); }))),
            entryComponents: __spreadArray([], __read(options.entryComponents))
        };
    }
        ...
        if (hasProviderOverrides || hasViewProviders) {
            ...
            if (hasProviderOverrides) {
                providerConfiguration = {
                    providers: __spreadArray(__spreadArray([], __read(options.componentProviders)), __read(options.componentMocks.map(function (p) { return options.mockProvider(p); })))
                };
            }
            if (hasViewProviders) {
                providerConfiguration = Object.assign(Object.assign({}, providerConfiguration), { viewProviders: __spreadArray(__spreadArray([], __read(options.componentViewProviders)), __read(options.componentViewProvidersMocks.map(function (p) { return options.mockProvider(p); }))) });
            }
    ...
    */
    ...
    function createHostFactory(typeOrOptions) {
        ...
        beforeEach(testing.waitForAsync(function () {
            ...
            overrideComponentIfProviderOverridesSpecified(options);
            if (options.template) {
                testing.TestBed.overrideComponent(options.host, {
        ...
        return function (template, overrides) {
            ...
            if (options.directiveProviders.length || options.directiveMocks.length) {
                testing.TestBed.overrideDirective(options.directive, {
                    set: { providers: __spreadArray(__spreadArray([], __read(options.directiveProviders)), __read(options.directiveMocks.map(function (p) { return options.mockProvider(p); }))) }
                });
            }
    ...
    */
    ...
    exports.toBePartial = toBePartial;
